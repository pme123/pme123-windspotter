...
object AuthService:
  ...
  def initialize(): Unit =
    ...
    if (storedToken != null && storedToken.nonEmpty) {
      if (storedToken.startsWith("verified_user_") || storedToken.startsWith("oauth_verified_")) {
        // Handle verified user session format
        val username = if (storedToken.startsWith("oauth_verified_")) {
          storedToken.replace("oauth_verified_", "")
        } else {
          storedToken.replace("verified_user_", "")
        }
  ...
  private def handleOAuthCallback(code: String): Unit =
    ...
    dom.console.log("🔐 GitHub OAuth completed successfully!")

    // Since we don't have a backend yet, we'll ask for username confirmation
    // This ensures the user actually completed GitHub OAuth
    val username = dom.window.prompt(
